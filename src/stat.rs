...
impl StatBasic for &[f64] {
    ...
    fn u_mean(self) -> f64 {
        let mut len: f64 = 0.0;
        return self.iter().fold(0.0, |acc: f64, ele| {len += 1.0; return acc + *ele; }) / len;
    }
    ...
    fn u_variance(self) -> f64 {
        ...
        let mut len: f64 = 0.0;
        let mut n: f64 = 0.0;
        return self.iter().fold(0.0, |sum, ele| {len += 1.0; n = *ele - mean; return sum + (n * n);}) / (len - 1.0);
    }
}
...
mod tests {
    ...
    fn u_mean_test() {
        ...
        let diff = abs([0.0, 0.25, 0.25, 1.25, 1.5, 1.75, 2.75, 3.25].as_ref().u_mean() - 1.375);
        assert!(diff <= EPSILON);
    }
